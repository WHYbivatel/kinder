(function () {
  ...
  function ai(text, opts) {
    opts = opts || {};
    return html(text || 'Подбираю...', Object.assign({ ai: true, inline: true }, opts));
  }

  function thinking(text) {
    return html(text || 'Думаю', {
      variant: 'dots',
  ...
  function aiRecommendations(text, count, opts) {
    ...
    const block = `<div class="loading-ui-ai-block">${ai(text || 'Подбираю...', { tag: false, compact: true })}${skeletonCards(count)}</div>`;
